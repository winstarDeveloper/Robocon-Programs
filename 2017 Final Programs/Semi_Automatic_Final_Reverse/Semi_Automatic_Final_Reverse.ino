...
unsigned short Max_Speed = 100/*250*/, Max_Speed_LF = 100;                             // set max motor rpm and line follower max rpm
...
double slowKp = 12.1, slowKi = 7.9, slowKd =  2.9;
//double slowestKp = 3.1, slowestKi = 4.9, slowestKd =  2.9;                  // marked
double slowestKp = 9.2, slowestKi = 4.9, slowestKd =  2.9;                  // marked
...
void lineFollow_frontSensor() {
  ...
  if (Output == 0) {
    ...
    if (Max_Speed_LF == 100) {
      Max_Speed_LF = Max_Speed_LF + 30;
    }
    if (Max_Speed_LF == 70) {
      Max_Speed_LF = Max_Speed_LF + 20;
    }
    if (Max_Speed_LF == 40) {
      Max_Speed_LF = Max_Speed_LF + 60;
    }
...
void rightSide() {                                              // turn the bot to right
  FLMot.LineReverse(100);
  LMot.LineReverse(100);
...
void leftSide() {                                               // turn the bot to left
  //  FLMot.Forward(Max_Speed);
  //  LMot.Forward(Max_Speed);
  FRMot.LineReverse(100);
  RMot.LineReverse(100);
  if (DEBUG) {
