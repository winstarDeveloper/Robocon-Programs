...
void lineFollow_backSensor() {
  ...
  if (Output == 0) {
    ...
    if (Max_Speed_LF == 40) {
      Max_Speed_LF = Max_Speed_LF + 50;
    }
    if (Max_Speed_LF == 50) {
      Max_Speed_LF = Max_Speed_LF + 40;
    }
    Right_Motor_Speed = Max_Speed_LF;
    if (DEBUG) {
      Serial.print("  Left " );
...
void loop() {
  ...
  if (command == 'A') {
    ...
    if (runPWM < maxIncSpeed && !accelerateOnce) {
      ...
      Max_Speed_LF = 40;//100;
      myPID.SetTunings(slowKp, slowKi, slowKd);
      myPID.SetOutputLimits(-20, 20); //-80, 80);
  ...
  if (command == 'Q') {
    ...
    */
    ...
    if (DEBUG) {
      Serial.println("Towards junction");
    }
  }
  if (automaticMode) {                                                          // Automatic bot line follow code here
    ...
    if (runPWM < maxIncSpeed && !accelerateOnce) {
      ...
      if (jCount_2 == 2) {
        ...
        jCount_2 = 0;
        automaticMode = false;
        normalStop(1);     // changed back motors hard stop removed
        //delay(200);
        setOnLine = true;
        // momentum carries bot
        //        Max_Speed_LF = 40;
        //        myPID.SetTunings(slowKp, slowKi, slowKd);
  ...
  if (setOnLine) {                                                              //auto adjustment of the bot on centre line
    ...
    lineFollow_backSensor();                                  // momentum is enough
