...
// stopped motor directly for center junction, momentum ka khel
...
boolean forwardON = false, reverseON = false;
...
// Front LSA08 Pins
#define analogPin A1
#define junctionPulse 2
unsigned short jCount = 0;
int readVal, positionVal, old_val, finalValue;

// Back LSA08 Pins
#define analogPin_2 A0
#define junctionPulse_2 3
...
void rightSide() {                                              // turn the bot to right
  maxIncSpeed = Max_Speed;
  if (runPWM < maxIncSpeed) {
    FLMot.LineForward(runPWM);
    LMot.LineForward(runPWM);
    runPWM++;
    delayMicroseconds(8000);
    if (DEBUG) {
    ...
    FLMot.LineForward(Max_Speed);
    LMot.LineForward(Max_Speed);
    //  FRMot.Reverse(Max_Speed);
    //  RMot.Reverse(Max_Speed);
    command = -1;
    if (DEBUG) {
...
void leftSide() {                                               // turn the bot to left
  maxIncSpeed = Max_Speed;
  if (runPWM < maxIncSpeed) {
    FRMot.LineForward(runPWM);
    RMot.LineForward(runPWM);
    runPWM++;
    delayMicroseconds(8000);
    if (DEBUG) {
    ...
    //  FLMot.Reverse(Max_Speed);
    //  LMot.Reverse(Max_Speed);
    FRMot.LineForward(Max_Speed);
    RMot.LineForward(Max_Speed);
    command = -1;
    if (DEBUG) {
...
void loop() {
  ...
  if (command == 'T') {
    if (forwardON) {
      FRMot.LineForward(Max_Speed - (Max_Speed / 2));
      RMot.LineForward(Max_Speed - (Max_Speed / 2));
      if (DEBUG) {
        Serial.print("Forward Directon - Right motor Speed set to ");
        Serial.println(Max_Speed - (Max_Speed / 2));
      }
    } else if (reverseON) {
      FRMot.LineReverse(Max_Speed - (Max_Speed / 2));
      RMot.LineReverse(Max_Speed - (Max_Speed / 2));
      if (DEBUG) {
        Serial.print("Reverse Direction - Right motor Speed set to ");
        Serial.println(Max_Speed - (Max_Speed / 2));
      }
    }
  }
  if (command == 'Y') {
    if (forwardON) {
      FLMot.LineForward(Max_Speed - (Max_Speed / 2));
      LMot.LineForward(Max_Speed - (Max_Speed / 2));
      if (DEBUG) {
        Serial.print("Forward Directon - Left motor Speed set to ");
        Serial.println(Max_Speed - (Max_Speed / 2));
      }
    } else if (reverseON) {
      FLMot.LineReverse(Max_Speed - (Max_Speed / 2));
      LMot.LineReverse(Max_Speed - (Max_Speed / 2));
      if (DEBUG) {
        Serial.print("Reverse Direction - Left motor Speed set to ");
        Serial.println(Max_Speed - (Max_Speed / 2));
      }
    }
  }
  ...
  if (automaticMode) {                                                          // Automatic bot line follow code here
    ...
    if (runPWM < maxIncSpeed && !accelerateOnce) {
      ...
      if (jCount_2 == 2) {
        if (DEBUG)
          Serial.println("Stopped on JCount 3");
        jCount_2 = 0;
        automaticMode = false;

        // stop normally momentum ka khel
        FLMot.LineReverse(0);
        LMot.LineReverse(0);
        FRMot.LineReverse(0);
        RMot.LineReverse(0);
        if(DEBUG){
          Serial.println("Motor Pwm all set to 0 (reverse dir)");
        }
        slowReverseStat = false;
        runPWM = 0;
  ...
  if (setOnLine) {                                                              //auto adjustment of the bot on centre line
    ...
    */
    ...
    //lineFollow_backSensor(); // stop normally momentum ka khel
