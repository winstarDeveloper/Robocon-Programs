...
// added pan on interrrupt with pot
...
void panCountInc() {
  ...
  if (interrupt_time - last_interrupt_time > debounce_time) {
    ...
    */
    ...
    if (panCountPot >= 1023 || panCount <= 0) { // set min and max values
      digitalWrite(panMot_1, HIGH);
      digitalWrite(panMot_2, HIGH);
      analogWrite(panPwm, 255);
      if (DEBUG) {
  ...
  if (autoAdjustCount != 0) {
    ...
    /*
      ...
      if (DEBUG) {
        ...
        Serial.print("\tPan Count Pot : ");
        Serial.print(panCountPot);
        Serial.print("\tStop Count : ");
        Serial.println(stopCount);
      }
      if (panCountPot < autoValues[autoAdjustCount][2]) {
        ...
        panCountPot = analogRead(panPot);
        sendDataToLcd(2);
      } else if (panCountPot > autoValues[autoAdjustCount][2]) {
        digitalWrite(panMot_1, HIGH);
      ...
      panCountPot = analogRead(panPot);
      sendDataToLcd(2);
      if (autoValues[autoAdjustCount][1] == angleVal && panCountPot == autoValues[autoAdjustCount][2]) {
        if (DEBUG) {
    ...
    */
    ...
    if (PS3.getButtonClick(CIRCLE)) {                                       // pan motor to left
      if (autoAdjust) {
        ...
        //panCountPot = analogRead(panPot);
        //sendDataToLcd(2);
        //if (panCountPot <= 1000 || panCountPot >= 24) {     // add limits by pot
        digitalWrite(panMot_1, HIGH);
        digitalWrite(panMot_2, LOW);
        analogWrite(panPwm, panPwmVal);
        //panCountPot = analogRead(panPot);
        ...
        //          analogWrite(panPwm, 255);
        //          if (DEBUG) {
        //            Serial.println("Pan overriding limits");
        //          }
        //panCountPot = analogRead(panPot);
        //sendDataToLcd(2);
        go = true;
