...
// disc loading sequence changed
// minute control of disc push
...
unsigned int rpmIncrementVal = 5;
String rmcsAngleSpeed = "S-100", rmcsAngleSpeedN = "S160"; // change to pwm values
static unsigned long discPushWaitTime = 1000, discWaitTime, discloadStartTime, discloadEndTime, throwingStarted, discloadingStartTime, startAutoAdjust;
...
boolean angleAdjust = false, sentRpm = false;

#define angleMot_1 29
#define angleMot_2 27
#define anglePwm 5
...
void loop() {
  ...
  if (PS3.PS3Connected || PS3.PS3NavigationConnected) {
    ...
    startAutoAdjust = millis();
    while (autoAdjust && autoAdjustCount != 0) {
      autoAdjustProc(autoAdjustCount);
      if (startAutoAdjust + 10000 < millis()) {
        if (DEBUG) {
          Serial.println("Saved by time");
        }
        digitalWrite(angleMot_1, HIGH);
        digitalWrite(angleMot_2, HIGH);
        analogWrite(anglePwm, 50);
    ...
    if (settingNextDisc) {
      ...
      if (discloadingStartTime + 450 < millis()) { // break loop if disc not loaded in 4 seconds
        if (DEBUG) {
    ...
    */
    if (PS3.getAnalogButton(L2) > 200) {
      ...
      analogWrite(discPullPwm, 125);
      if (DEBUG) {
        Serial.println("Release Disc's - fast");
      }
      buttonsL2 = true;
    } else if (PS3.getAnalogButton(L2) < 200 && PS3.getAnalogButton(L2) > 10) {
      digitalWrite(discPullMot_1, LOW);
      digitalWrite(discPullMot_2, HIGH);
      analogWrite(discPullPwm, 50);
      if (DEBUG) {
        Serial.println("Release Disc's - slow");
      }
      buttonsL2 = true;
    }
    if (!PS3.getButtonPress(L2) && buttonsL2) {
    ...
    if (PS3.getAnalogButton(R2) > 200) {
      ...
      analogWrite(discPullPwm, 125);
      if (DEBUG) {
        Serial.println("Push Disc's - fast");
      }
      buttonsR2 = true;
    } else if (PS3.getAnalogButton(R2) < 200  && PS3.getAnalogButton(R2) > 10) {
      digitalWrite(discPullMot_1, HIGH);
      digitalWrite(discPullMot_2, LOW);
      analogWrite(discPullPwm, 50);
      if (DEBUG) {
        Serial.println("Push Disc's - slow");
      }
      buttonsR2 = true;
    }
    if (!PS3.getButtonPress(R2) && buttonsR2) {
